...
targets <- readTargets("log_targets.txt")
...
for (contrast in contrasts) {
    ...
    write.table(output, file=paste("data/log_contrasts/", contrast, ".txt", sep=""), sep="\t", quote=FALSE)

    output <- topTable(fit2, adjust="BH", coef=contrast, genelist=y.ave$genes, number=10)
    write.table(output, file=paste("data/log_contrasts/", contrast, "_top10.txt", sep=""), sep="\t", quote=FALSE)

    results <- decideTests(fit2)

    png(paste("lplots/log/ma_", contrast, ".png", sep=""),
        width=14, height=14, units = 'cm', res=300)
    plotMA(fit2, 1)
    dev.off()

    png(paste("lplots/log/volcano_", contrast, ".png", sep=""),
        width=14, height=14, units = 'cm', res=300)
